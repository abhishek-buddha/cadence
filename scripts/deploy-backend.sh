...
echo "Dry-run check first (no verbose flag — avoids printing secrets to the terminal)..."
CONVEX_DEPLOY_KEY="$CONVEX_DEPLOY_KEY" npx convex deploy --dry-run

echo ""
read -p "Dry-run looked clean above? Proceed with the real deploy to production? [y/N] " confirm
